...
//             if (m_modifiers & Qt::ShiftModifier) cout << " Shift";
//             if (m_modifiers & Qt::AltModifier) cout << " Alt";
//             if (m_modifiers & Qt::ControlModifier) cout << " Control";
//             if (m_modifiers & Qt::MetaModifier) cout << " Meta";
//             if (m_modifiers & Qt::KeypadModifier) cout << " Keypad";
//             if (m_modifiers & Qt::GroupSwitchModifier) cout << " GroupSwitch";
...
bool
QTTranslator::sendKeyEvent(QEvent* qevent) const
{
    ostringstream str;
    ...
    {
        if (event->isAutoRepeat() && event->type() == QEvent::KeyRelease) return true;
    }
    ...
    //  XXX we _still_ have problems with release event modifiers not matching
    //  press event.  This might fix it (for later):
    //  
    ...
    {
      case Qt::Key_Shift:
          if (keydown) m_modifiers |= Qt::ShiftModifier;
          else m_modifiers &= ~Qt::ShiftModifier;
          break;

      case Qt::Key_Control:
          if (keydown) m_modifiers |= Qt::ControlModifier;
      ...
      case Qt::Key_Super_R:
          meta = true;
          if (keydown) m_modifiers |= Qt::MetaModifier;
          else m_modifiers &= ~Qt::MetaModifier;
          break;

      case Qt::Key_Alt:
          alt = true;
          if (keydown) m_modifiers |= Qt::AltModifier;
    ...
    if (utf8string.size() == 1)
    {
        ...
        if (isupper(rkey) || ispunct(rkey)) showShift = false;
    ...
    if (nokeypad) mods &= ~Qt::KeypadModifier;

    str << modifierString(mods, showShift);

    switch (event->key())
    {
      case Qt::Key_Shift:       str << "-shift"; break;
      case Qt::Key_Control:     str << "-control"; break;
      case Qt::Key_Meta:        str << "-meta"; break;
      case Qt::Key_Alt:         str << "-alt"; break;
      case Qt::Key_Escape:      str << "-escape"; break;
      case Qt::Key_Tab:         str << "-tab"; break;
      case Qt::Key_Backtab:     str << "-tab"; break;
      case Qt::Key_Backspace:   str << "-backspace"; break;
      case Qt::Key_Return:      str << "-enter"; break;
      case Qt::Key_Enter:       str << "-keypad-enter"; break;
      case Qt::Key_Insert:      str << "-insert"; break;
      case Qt::Key_Delete:      str << "-delete"; break;
      case Qt::Key_Pause:       str << "-pause"; break;
      case Qt::Key_Print:       str << "-print"; break;
      case Qt::Key_SysReq:      str << "-sysreq"; break;
      case Qt::Key_Clear:       str << "-clear"; break;
      ...
      case Qt::Key_AltGr:       str << "-alt-gr"; alt = true; break;
      case Qt::Key_CapsLock:    str << "-caplock"; break;
      case Qt::Key_NumLock:     str << "-numlock"; break;
      case Qt::Key_ScrollLock:  str << "-scrolllock"; break;
      case Qt::Key_Help:        str << "-help"; break;
      case Qt::Key_Space:       str << "- "; break;
      case Qt::Key_Super_L:     str << "-super-left"; break;
      case Qt::Key_Super_R:     str << "-super-right"; break;
      case Qt::Key_Hyper_L:     str << "-hyper-left"; break;
      case Qt::Key_Hyper_R:     str << "-hyper-right"; break;
      case Qt::Key_Direction_L: str << "-direction-left"; break;
      case Qt::Key_Direction_R: str << "-direction-right"; break;
      case Qt::Key_Menu:        str << "-menu"; break;
      case 0:
      case Qt::Key_unknown:     str << "-unknown"; break;
      default:
    ...
    unsigned int key = (alt || ucs4string.empty()) 
                            ? event->key() 
                            : ucs4string.front();

    if (keydown)
    {
        KeyPressEvent e(str.str(), 
                        m_node, 
                        key,
                        modifiers(mods));
...
string
QTTranslator::pointerString(bool b1, bool b2, bool b3, 
                            ...
                            bool tabletPushed = false) const
{
    ...
    if (Options::sharedOptions().stylusAsMouse)
    {
        switch (eventType)
        {
          case QEvent::MouseButtonPress:    n += "push"; break;
          case QEvent::MouseButtonRelease:  n += "release"; break;
          case QEvent::MouseButtonDblClick: n += "double"; break;
          case QEvent::MouseMove:           n += (buttons) ? "drag" : "move"; break;
          case QEvent::Enter:               n += "enter"; break;
          case QEvent::Leave:               n += "leave"; break;
          case QEvent::WindowActivate:      n += "activate"; break;
          default:
            break;
        }
    }
...
bool
QTTranslator::sendMouseWheelEvent(QEvent* qevent) const
{
    ...
    n += "wheel";
    if (event->delta() < 0) n += "down"; else n += "up";
...
QTTranslator::sendMouseEvent(QEvent* qevent, float activationTime) const
{
    ...
    else if (type == QEvent::MouseButtonPress || type == QEvent::MouseButtonDblClick)
    {
        ...
        PointerButtonPressEvent event(n, m_node, mods,
                                      m_x, m_y,
                                      m_width, m_height,
                                      m_x, m_y,
                                      buttons(), 0,
    ...
    else if (type == QEvent::MouseButtonRelease)
    {
        ...
        n = pointerString(m_b1, m_b2, m_b3, 
                          QEvent::MouseButtonRelease, mods, ebuttons);

        PointerButtonReleaseEvent event(n, m_node, modifiers(mods),
                                        m_x, m_y,
                                        m_width, m_height,
                                        m_pushx, m_pushy,
                                        buttons());
        ...
        if (b1 || b2 || b3) 
        {
            n = pointerString(b1, b2, b3, 
                              QEvent::MouseButtonPress, mods, ebuttons);

            PointerButtonPressEvent event(n, m_node, modifiers(mods),
                                          m_x, m_y,
                                          m_width, m_height,
                                          m_x, m_y,
                                          buttons());
    ...
    {
        if ((b1 != m_b1 || b2 != m_b2 || b3 != m_b3 || m_lastMods != m_modifiers)
            && !m_firstTime)
        {
            n = pointerString(m_b1, m_b2, m_b3,
                              QEvent::MouseButtonRelease,
                              m_lastMods,
                              qbuttons(m_lastButtons));

            PointerButtonReleaseEvent eEnd(n, m_node, modifiers(m_modifiers),
                                           m_x, m_y,
                                           m_width, m_height,
                                           m_pushx, m_pushy,
                                           m_lastButtons);
            ...
            PointerButtonPressEvent eBegin(n, m_node, modifiers(mods),
                                           m_x, m_y,
                                           m_width, m_height,
                                           m_x, m_y,
                                           buttons());
...
unsigned int
QTTranslator::buttons() const
{
    return int(m_b1) | (int(m_b2) << 1) | (int(m_b3) << 2);
}

string
QTTranslator::modifierString(Qt::KeyboardModifiers mods, bool showShift) const
{
    string s;
    unsigned int m = modifiers(mods);

    if (m & ModifierEvent::Alt)         s += "-alt";
    if (m & ModifierEvent::CapLock)     s += "-caplock";
    if (m & ModifierEvent::Control)     s += "-control";
    if (m & ModifierEvent::Meta)        s += "-meta";
    if (m & ModifierEvent::NumLock)     s += "-numlock";
    if (m & ModifierEvent::ScrollLock)  s += "-scrolllock";
    
    if (showShift &&
        m & ModifierEvent::Shift)       s += "-shift";
...
bool 
QTTranslator::sendDNDEvent(QEvent* event) const
{
    if (QDropEvent *de = dynamic_cast <QDropEvent *> (event))
    {
        //
        //  Ignore attempts by the dropper to delete the dropped file after RV accepts the drop.
        //  NOTE: on windows MoveAction means the explorere will delete the file after the drop.
        //  on mac, MoveAction seems to stand in for CopyAction sometimes, so just treat everything
        //  as a copy.
        //
...
QTTranslator::sendTabletEvent(QEvent* qevent) const
{
    ...
    TabletEvent e(n, m_node, modifiers(mods),
                  m_x, m_y,
                  m_width, m_height,
                  //m_lastx, m_lasty,
                  m_pushx, m_pushy,
...
void 
QTTranslator::setScaleAndOffset(float x, float y, float sx, float sy) const
{
    m_xoffset = x;
    m_yoffset = y;
    m_xscale = sx;
...
void 
QTTranslator::setRelativeDomain(float w, float h) const
{
    m_width = w;
    m_height = h;
}
